...
import { EstadoEvento } from './event-status.enum';
...
export class CrearEventoDto {
  ...
  @IsString() horaApertura: string;
  @IsString() horaInicio: string;
  @IsString() horaCierre: string;
  @IsString() lugar: string;
  @IsString() direccion: string;
  @IsOptional() @IsString() flyerPrincipal?: string;
  @IsInt() @Min(1) capacidadTotal: number;
  @IsInt() @Min(13) edadMinima: number;
  @IsEnum(EstadoEvento) estado: EstadoEvento;
